...
public:
	...
	static std::vector<ColliderComponent*> GetOverlappingColliders(Collider* collider);
	static std::vector<ColliderComponent*> ConeCast(Vector2 origin, Vector2 direction, float angle, float radius, CollisionFilter filter);

	static std::vector<ColliderComponent*> CheckCollisionAt(Collider* collider, Vector2 newPosition, CollisionFilter filter);
	...
	static void DebugDrawPolygon(const std::vector<Vector2>& polygon, float lifetime, int particlesPerEdge = 20);
